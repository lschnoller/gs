				...
				<div class="content-box-content">
					...
					<div class="tab-content <?=$listing?>" id="listing"> <!-- This is the target div. id must match the href of this div's tab -->	
						...
						<table>
							<thead>
								<?php 
								echo list_head('admin_'.$me,'Author','author');
								?>
							</thead>
							...
							<tbody>
								...
								if(is_array($list)) {
									foreach ($list as $key=>$val){
										echo list_item($val['status'],$key,'admin_'.$me,$val['author']);
									}
								}
								?>							
							</tbody>
					...
					<div class="tab-content <?=$edit?>" id="addedit">
						...
						<form action="?" method="post"  enctype="multipart/form-data" >
							...
							<fieldset> 
								<?php							
								echo form_input(true,'Author','author',set_value('author',$values['author']));
								echo form_textarea(true,'Quote Text','quote_text',set_value('quote_text',$values['quote_text']));
								echo form_input(false,'Profesión','profession', set_value('profession', $values['profession']));
								echo form_hr();
